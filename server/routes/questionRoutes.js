...
router.route("/:id").put(async (req, res) => {
    ...
    try {
        ...
        const questionColumn = map.qstColumns.find((column) => column._id.toString() === questionColumnId);
        if (!questionColumn) {
            return res.status(404).json({ success: false, message: "Question column not found" });
        }

        // find question by questionId and update
        const question = questionColumn.questions.find((question) => question._id.toString() === questionId);
        if (!question) {
